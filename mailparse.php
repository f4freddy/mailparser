...
class PlancakeEmailParser {
  ...
  public function get_header_data($string, $start, $end, $index, $num) {
    ...
    if ($data) {
      ...
      $senderHostName = $this->get_string_between($data, '(', '[');
      $s = explode('(', $data);
    ...
    $time = (count($timeArray) > 1 ) ? gmdate('d/m/Y H:i:s', strtotime(end($timeArray))) . ' UTC ' : 'Invalid receive time';
    ...
    $overview = array(
        'Entry' => $index,
        'Sender' => ($senderHostName) ? ($senderHostName) : 'Not available',
        'Received By' => ($by) ? $by : 'Not available',
    ...
    $details = array(
        $entryKey => $string,
        'Sender' => ($senderHostName) ? $senderHostName : 'Not available',
        'Sender Host Name' => ($senderHostName) ? $senderHostName : 'Not available',
        'Sender IP Address' => ($senderIp) ? $senderIp : 'Not available',
        'Sender (from)' => ($senderAdreess) ? $senderAdreess : 'Not available',
        'Received By' => ($by) ? $by : 'Not available',
        'Received With' => ($With) ? $With : '',
        'Receive Times' => ($time) ? $time : 'Not available',
        'Receive Duration' => 'N/A'
    );
  ...
  public function getAllHeader() {
    ...
    $this->rawFields['received'] = $this->recivedHeaders[1];
    $this->rawFields['details'] = $senderDetails;
